...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if($validator->emptyCheck($fullname, $username, $password, $confirm)){
	  if($db->query("SELECT * FROM admin WHERE username='$username'")->num_rows == 0){
		  if($password === $confirm){
			  $password = sha1($password);
			  if($db->query("INSERT INTO admin(name, username, password, last) VALUES ('$fullname','$username','$password','None')")){
