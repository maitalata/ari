...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if($validator->emptyCheck($title, $description)){
	if($db->query("UPDATE activities SET title='$title', description='$description' WHERE id='$id' ")){
		$response = "<div class=\"w3-panel w3-green w3-leftbar \">
						<h6><i class=\"fa fa-check w3-green w3-margin-right\"></i> Activity updated successfully</h6>
						</div>";
		...
		$response = "<div class=\"w3-panel w3-red w3-leftbar \">
						<h6><i class=\"fa fa-times w3-deep-orange w3-margin-right\"></i> System unable to update activity</h6>
						</div>";
	}
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <header class="w3-container" style="padding-top:22px">
    <h3><b><i class="fa fa-edit"></i> Edit Activity</b></h3>
  </header>
	<form action="" enctype="multipart/form-data" method="POST">
	<div class="w3-panel">
		<?php echo $response; ?>
		  <label>Activity Title</label>
		  <input class="w3-input w3-border w3-margin-bottom" type="text" placeholder="Activity Title" name="title" value="<?php echo $row['title']; ?>"  required >
		  <label>Picture 1</label>
		  <input class="w3-input w3-border w3-margin-bottom" type="file" name="picture1" >
		  <label>Picture 2</label>
		  <input class="w3-input w3-border w3-margin-bottom" type="file" name="picture2" >
		  ...
		  <textarea class="w3-input w3-border w3-margin-bottom" placeholder="Description of activity" name="description" rows="4" required ><?php echo $row['description']; ?></textarea>
		  <div class="w3-section"> 
			<button type="submit" class="w3-button w3-left w3-theme-d3" title="Add Activity">Update</button>
		  </div>
    </div>
	</form>
